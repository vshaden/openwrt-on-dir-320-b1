...
# odpytanie urzadzenia -опрос устройства
...
# znajdz odpowiednia sekcje w konfiguracji-Найти соответствующие разделы в конфигурации
getpath ${DEVICE##/*/}
PORIG=$P
...
# daj pin jak jest taka potrzeba -При необходимости ввести пин-код
if [ ! -f /tmp/pincode_was_given ]; then
	# tylko za pierwszym razem - только для первого раза
	if [ ! -z $SEC ]; then
...
# dla modemów Option i ZTE - Для модемов Option и ZTE
if [ "$COPS_NUM" = "-" ]; then
...
# Nowe Huawei - Новые Huawei
TECH=$(echo "$O" | awk -F[,] '/^\^SYSINFOEX/ {print $9}' | sed 's/"//g')
if [ "x$TECH" != "x" ]; then
	MODE="$TECH"
fi

# Starsze modele Huawei i inne pozostale - Старые модели Huawei и другие 
if [ "x$MODE" = "x-" ]; then
...
# Status polaczenia - Статус подключения
...
# Informacja o urzadzeniu Информация об устройстве 
DEVICE=$(echo "$O" | awk -F[:] '/DEVICE/ { print $2}')
if [ "x$DEVICE" = "x" ]; then
	DEVICE="-"
fi

# podmiana w szablonie  подстановки в шаблоне
if [ $TOTXT -eq 0 ]; then
	EXT="html"
else
	EXT="txt"
