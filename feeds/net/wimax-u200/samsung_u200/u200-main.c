...
static char essid[32] = "@aksoran.kz";
//static char essid[32] = "@mezon.lt";
//static char essid[32] = "@wimax.comstar-uts.ru";
module_param_string(essid, essid, sizeof(essid), 0444);
MODULE_PARM_DESC(essid, "WIMAX ESSID");
...
static void u200_read_bulk_callback(struct urb *urb)
{
	...
	if (urb->status) {
		if(!(urb->status == -ENOENT ||
		    urb->status == -ECONNRESET ||
		    urb->status == -ESHUTDOWN))
			pr_err("%s - nonzero read bulk status received: %d:async read stoped",
			    __func__, urb->status);
		rerun=0;
	}
...
static int u200_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	if(retval<len)
	{
		pr_err("can`t send init1 request");
		goto init_error;
	}
	...
	if(retval<len)
	{
		pr_err("Can`t send string info request\n");
		goto init_error;
	}
	...
	if(retval<len)
	{
		pr_err("Can`t send init2 request\n");
		goto init_error;
	}
	...
	if(retval<len)
	{
		pr_err("Can`t send init3 request\n");
		goto init_error;
	}
	...
	if(retval<len)
	{
		pr_err("Can`t send authorization_data request\n");
		goto init_error;
	}
